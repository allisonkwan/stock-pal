...
export default function App() {
  ...
  masterDataPoints = [datapoint0, datapoint1, datapoint2, datapoint3, datapoint4, datapoint5, datapoint6, datapoint7, datapoint8, datapoint9, datapoint10, datapoint5, datapoint11];
...
//   });
  ...
  // const oldMentionsData = useRef([0, 0, 0, 0, 0]);

  // var mentionsData = [6, 7, 8, 9, 10];


  function filterChart(time) {
    console.log("CALLED FILTERCHART");
    console.log(time);

    if (radioButtons == '1D') {
      // console.log("doing a day");
      ...
      apiRequest.onload = function () {
        ...
        // mentionsData = [point0, point1, point2, point3, point4];
        setMentionsData(() => [point0, point1, point2, point3, point4]);
        // oldMentionsData.current = [point0, point1, point2, point3, point4];

      }
      // mentionsData = [1, 2, 3, 4, 5];
      ...
      apiRequest.onload = function () {
        ...
        // mentionsData = [point0, point1, point2, point3, point4, point5, point6];

      // stock price call
      //ApiRequest('AAPL', '1', 'week', last_week_string, today_string);
        // mentionsData = [point0, point1, point2, point3, point4, point5, point6];
        setMentionsData(() => [point0, point1, point2, point3, point4, point5, point6]);
        // oldMentionsData.current = [point0, point1, point2, point3, point4, point5, point6];

      }
      
      // mentionsData = [0, 1, 2, 3, 4, 5, 6];
      ...
      apiRequest.onload = function () {
        ...
        // setMentionsData([temp1, temp2, point0, point1, point2]);
      }
    ...
    // setTimeout(
    //   () => alert(`state: ${mentionsData} | ref: ${oldMentionsData.current}`),
    //   1000
    // );
    ...
    for (let i = 0; i < mentionsData.length; i++) {
      dataPoints.push(new DataPoint(masterDataPoints[i].timestamp, mentionsData.current[i], masterDataPoints[i].cost, masterDataPoints[i].googleData, masterDataPoints[i].redditData, masterDataPoints[i].twitterData));
    }
    switch (time) {
      case '1D':
        // setTestData({
    ...
    // setTimeout(
    //   () => alert(`state: ${testData} | ref: ${oldTestData.current}`),
    //   1000
    // );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        <MyLineChart data={oldTestData.current} />
      </ScrollView>
    </SafeAreaView>
  );
}
